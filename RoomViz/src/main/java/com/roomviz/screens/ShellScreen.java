...
public class ShellScreen extends JPanel {
    ...
    public ShellScreen(AppFrame frame, Router outerRouter) {
        ...
        innerRouter.add(ScreenKeys.DASHBOARD, new DashboardPage(frame, innerRouter));

        // ✅ REAL pages (no placeholders)
        innerRouter.add(ScreenKeys.DESIGN_LIBRARY, new DesignLibraryPage(frame, innerRouter));
        innerRouter.add(ScreenKeys.NEW_DESIGN, new NewDesignWizardPage(frame, innerRouter));

        // ✅ Design tools
        innerRouter.add(ScreenKeys.PLANNER_2D, new Planner2DPage(frame));
        innerRouter.add(ScreenKeys.VIEW_3D, new Visual3DPage(frame, innerRouter));
        innerRouter.add(ScreenKeys.SHADING_COLOR, new ShadingColorPage(frame));

        // ✅ Settings
        innerRouter.add(ScreenKeys.SETTINGS, new SettingsPage(frame));

        // Sidebar (ALL navigation here) ✅ keep as-is
