...
/**
 * Lightweight, read-only 3D-ish preview renderer for DesignDetailsPage.
 *
 * ✅ Fixes:
 * - L-Shape rooms now render as L-shaped floor (not a square)
 * - Furniture now renders as actual shapes (chair seat+back, round-table cylinder)
 * - Furniture icon text rendered on the top surface
 *
 * Notes:
 * - Uses Design.layoutX/layoutY/layoutWidth/layoutHeight to map 2D canvas coords -> room coords
 * - Isometric projection (no interaction)
 */
